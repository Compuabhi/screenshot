...
//callback after loading of page
  addScreen: function (data) {
    if (api.stop) return;
    screenshot.retries++;
    //sending message from extension to content script corresponding tab // intab
  ...
  ans: function (mess) {
    ...
    if (!mess && chrome.runtime.lastError) {
      if (screenshot.retries > 1 && screenshot.scroll) {
        api.callPopup({type: 'message', message: 'Sorry, can not take a full screenshot of this webpage. This might be because it is not fully loaded. Please report this issue.'});
        return ;
  ...
  // after mess is completed screenshot is created
